...
class Model {
    ...
    void forNeighbors(Place place, Consumer<Place> action) {
        ...
        if (j == 0 && i < _height - 1 && i > 0) {
            action.accept(pl(i + 1, j));
            action.accept(pl(i - 1, j));
            action.accept(pl(i, j + 1));
        }
        if (j == _width - 1 && i < _height - 1 && i > 0) {
            action.accept(pl(i + 1, j));
            action.accept(pl(i - 1, j));
            action.accept(pl(i, j - 1));
        }
    ...
    boolean[] colorsPresent() {
        boolean[] x = new boolean[_ncolors];
        int[] y = new int[_ncolors];
        int k = 0;
        while (k < _ncolors) {
            y[k] = k;
            k += 1;
    ...
    Set<Place> adjacentCells(int color) {
        HashSet<Place> adjacentcell = new HashSet<>();
        for (Place i: _active) {
            forNeighbors(i, (adjacent) -> {
                if (get(adjacent) == color) {
                    adjacentcell.addAll(findRegion(adjacent, color,
                            new HashSet<>()));
                }
            });
        }
